...
/*------------------------------ D1/CIF <h.264> + QCIF <h.264> + D1 <MJPEG> -------------------------------------
              ...
              ------------------------------------------------------------------------------------
                  ...
                  |                  |                           |                       |
                  |        -------------------------     ----------------------    ---------------------------
           16D1 420 30fps       DEI [SC+VIP0SC]             DEI [SC+VIP0SC]               DEI [SC2+VIP1SC]
                  |                 <DEI-SC>              <VIP0-SC>   <DEI-SC>            <VIP1-SC>
           (Even fields)   -------------------------     ----------------------    ---------------------------
                  |                  | 4D1               | 16D1       | 16D1 MJPEG        | 16QCIF
                  |                  | 30/60fps          | 7.5fps     | 1fps              | 30fps
                  |                  |                   | (YUV420)   | (YUV420)          | (YUV422)
                  |                  |                   |            |                   |
                  |                  |                   |            |                   |
                  |                  |                   |            |NSF_AFTER_SCALAR   |
                  |                  |                   |            |(YUV420)           |
...
#define     NUM_DEI_LIVE_BUFFERS        4
#define     NUM_DEI_D1_BUFFERS          3
#define     NUM_DEI_QCIF_BUFFERS        4
...
#ifdef     ADD_NSF_AFTER_CAPTURE
    gVcapModuleContext.nsfId[1]     = 0xFFFFFFFF;
#endif
#ifdef      ADD_DEI_FOR_1_AND_4CH_60_FPS
...
Void setDspLinksParamsNrt(IpcFramesOutLinkRTOS_CreateParams    *ipcFramesOutVpssPrm,
...
Void MultiCh_createProgressive16ChNrtVcapVencVdecVdis()
{
    ...
    setLinkIdsNrt(enableOsdAlgLink, mergeId, dupId, ipcId, selectId);
    ...
    nsfPrm.inQueParams.prevLinkId        = gVcapModuleContext.deiId[DEI_VIP0_SC_DEI_SC_2];
    nsfPrm.inQueParams.prevLinkQueId     = DEI_LINK_OUT_QUE_DEI_SC;
    nsfPrm.numOutQue                     = 1;
    nsfPrm.outQueParams[0].nextLink      = mergeId[D1_CIF_QCIF_MERGE_LINK_IDX];
    ...
    mergePrm[D1_CIF_QCIF_MERGE_LINK_IDX].inQueParams[2].prevLinkId       = gVcapModuleContext.nsfId[NSF_AFTER_DEI_SC_LINK_IDX];
    ...
    if (enableOsdAlgLink || enableScdAlgLink)
    {
        ...
        setDspLinksParamsNrt(&ipcFramesOutVpssPrm, &ipcFramesInDspPrm, mergeId[D1_CIF_QCIF_MERGE_LINK_IDX], 0, ipcId);
    ...
    if (enableScdAlgLink)
    {
        ...
        if(Vcap_isPalMode())
           ...
           dspAlgPrm.scdCreateParams.maxHeight              = 120;
        ...
        else   /* For 144 Block height becomes 12 */
    ...
    System_linkCreate(gVcapModuleContext.nsfId[NSF_AFTER_DEI_SC_LINK_IDX] , &nsfPrm, sizeof(nsfPrm));
