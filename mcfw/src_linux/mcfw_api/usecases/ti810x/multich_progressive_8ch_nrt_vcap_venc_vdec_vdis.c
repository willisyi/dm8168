...
/*------------------------------ D1/CIF <h.264> + QCIF <h.264> + D1 <MJPEG> -------------------------------------
              ...
              ------------------------------------------------------------------------------------------------------------------
                  |                  |                           |                       |                            |
                  |                  |                           |                       |                            |
                  |                  | 8D1                       | 8D1                   | 8D1 420 30fps              | 8D1 420 30fps
                  |             ------------                     | 15fps                 | (Even fields)              | (Even fields)
                  |             Select Link                      |                       |                            |
                  |             ------------                     |                       |                            |
                  |                  | 4D1                       |                       |                            |
                  |                  |                           |                       |                            |
                  |        -------------------------     ----------------------    --------------------------- --------------------
           8D1 420 30fps        DEI [SC+VIP0SC]             DEI [SC+VIP0SC]               DEI [SC2+VIP1SC]         SCLR [SC5]
                  |                 <DEI-SC>                    <VIP0-SC>                     <VIP1-SC>
           (Even fields)   -------------------------     ----------------------    --------------------------- --------------------
                  |                  | 4D1                       | 8D1                   | 8QCIF                      | 8D1 MJPEG
                  |                  | 30/60fps                  | 15fps                 | 30fps                      | 1fps
...
// For 8ch QCIF
static Void set_DEI_BYPASS_VIP1_SC_SC2_paramsNrt(  
        ...
        Uint32                nextLinkIdFor_VIP_SC)
{
    ...
    deiPrm->tilerEnable[DEI_LINK_OUT_QUE_VIP_SC] = FALSE; //TILER_ENABLE; [TODO] SCD doesn't work in Tile at the moment. WIP
    deiPrm->comprEnable          = FALSE;
    deiPrm->setVipScYuv422Format = FALSE;
    deiPrm->enableLineSkipSc     = FALSE;

    deiPrm->inputFrameRate[DEI_LINK_OUT_QUE_VIP_SC]  = Vcap_isPalMode() ?  25 : 30;
    deiPrm->outputFrameRate[DEI_LINK_OUT_QUE_VIP_SC] = Vcap_isPalMode() ?  25 : 30;
        ...
        UInt32                   deiId)
{
    ...
    deiPrm->inputFrameRate[DEI_LINK_OUT_QUE_DEI_SC]  = Vcap_isPalMode() ? 50 : 60;
    deiPrm->outputFrameRate[DEI_LINK_OUT_QUE_DEI_SC] = Vcap_isPalMode() ? 50 : 60;
...
// For 8ch D1 15 fps
static Void set_DEI_VIP0_SC_DEI_SC_2_paramsNrt(       
        DeiLink_CreateParams *deiPrm,
        Uint32                prevLinkId,
        Uint32                prevLinkQ,
        Uint32                nextLinkIdFor_DEI_SC,
        Uint32                nextLinkIdFor_VIP_SC)
{
    ...
    deiPrm->inputDeiFrameRate  = Vcap_isPalMode() ? 50 : 60;
#else
    deiPrm->inputDeiFrameRate  = Vcap_isPalMode() ? 25 : 30;
#endif
    deiPrm->outputDeiFrameRate = Vcap_isPalMode() ? 12 : 15;

    deiPrm->inputFrameRate[DEI_LINK_OUT_QUE_DEI_SC]  = Vcap_isPalMode() ? 12 : 15;
    deiPrm->outputFrameRate[DEI_LINK_OUT_QUE_DEI_SC] = 1;

    deiPrm->inputFrameRate[DEI_LINK_OUT_QUE_VIP_SC]  = Vcap_isPalMode() ? 12 : 15;
    deiPrm->outputFrameRate[DEI_LINK_OUT_QUE_VIP_SC] = Vcap_isPalMode() ? 12 : 15;
...
static Void setMJPEGEncChParamsNrt(
...
static Void setH264EncChParamsNrt(
        EncLink_CreateParams *encPrm, 
        Int32                 startChNum, 
        Int32                 endChNum)
{
...
Void MultiCh_createProgressive8ChNrtVcapVencVdecVdis()
{
    ...
    enableAlgLink = gVsysModuleContext.vsysConfig.enableOsd;
    ...
    nsfPrm[NSF_AFTER_CAPTURE_LINK_IDX].numBufsPerCh              = 
        NUM_CAPTURE_BUFFERS - 2;
    nsfPrm[NSF_AFTER_CAPTURE_LINK_IDX].inputFrameRate            = 
        Vcap_isPalMode() ? 25 : 30;
    nsfPrm[NSF_AFTER_CAPTURE_LINK_IDX].outputFrameRate           = 
        Vcap_isPalMode() ? 25 : 30;

    dupPrm[CAPTURE_DUP_LINK_IDX].inQueParams.prevLinkId    = 
        gVcapModuleContext.nsfId[NSF_AFTER_CAPTURE_LINK_IDX];
    ...
    dupPrm[CAPTURE_DUP_LINK_IDX].outQueParams[2].nextLink  = 
        gVcapModuleContext.deiId[DEI_VIP0_SC_DEI_SC_2];
    ...
    set_DEI_VIP0_SC_DEI_SC_paramsNrt(
    ...
    set_DEI_VIP0_SC_DEI_SC_2_paramsNrt(
            &deiPrm[DEI_VIP0_SC_DEI_SC_2],
            dupId[CAPTURE_DUP_LINK_IDX],  
            2,
            SYSTEM_LINK_ID_INVALID,
    ...
    /*************** SC2 / VIPSC Settings ******************/
    set_DEI_BYPASS_VIP1_SC_SC2_paramsNrt(
    ...
    sclrPrm.inputFrameRate                               = Vcap_isPalMode() ?  25 : 30;
    ...
    mergePrm[D1_CIF_QCIF_MERGE_LINK_IDX].inQueParams[2].prevLinkId    = 
        gVcapModuleContext.nsfId[NSF_AFTER_SC5_LINK_IDX];
    ...
    dspAlgPrm.enableOSDAlg = enableOsd;
        ...
        ipcOutVpssPrm.inQueParams.prevLinkId                       = 
            mergeId[D1_CIF_QCIF_MERGE_LINK_IDX];
    ...
    {
        dspAlgPrm.outQueParams[ALG_LINK_SCD_OUT_QUE].nextLink = 
            SYSTEM_LINK_ID_INVALID;
    }
    ...
    System_linkCreate(
            gVcapModuleContext.nsfId[NSF_AFTER_CAPTURE_LINK_IDX], 
            &nsfPrm[NSF_AFTER_CAPTURE_LINK_IDX], 
            sizeof(nsfPrm[NSF_AFTER_CAPTURE_LINK_IDX]));
    System_linkCreate(
            dupId[CAPTURE_DUP_LINK_IDX], 
    ...
    System_linkCreate(
            gVcapModuleContext.deiId[DEI_VIP0_SC_DEI_SC_2], 
            &deiPrm[DEI_VIP0_SC_DEI_SC_2], 
            sizeof(deiPrm[DEI_VIP0_SC_DEI_SC_2]));
    System_linkCreate(
            gVcapModuleContext.deiId[DEI_BYPASS_VIP1_SC_SC2], 
            &deiPrm[DEI_BYPASS_VIP1_SC_SC2], 
    ...
    //        &nsfPrm[NSF_AFTER_DEI_SC_LINK_IDX], 
    //        sizeof(nsfPrm[NSF_AFTER_DEI_SC_LINK_IDX]));
    System_linkCreate(
            gVcapModuleContext.sclrId[0], 
            &sclrPrm, 
            sizeof(sclrPrm));
    System_linkCreate(
            gVcapModuleContext.nsfId[NSF_AFTER_SC5_LINK_IDX], 
            &nsfPrm[NSF_AFTER_SC5_LINK_IDX], 
            sizeof(nsfPrm[NSF_AFTER_SC5_LINK_IDX]));
    System_linkCreate(
            mergeId[D1_CIF_QCIF_MERGE_LINK_IDX], 
...
Void MultiCh_deleteProgressive8ChNrtVcapVencVdecVdis()
{
    UInt32 i;
    UInt32 mergeId[NUM_MERGE_LINK];
    UInt32 dupId[NUM_DUP_LINK];
...
Int32 MultiCh_progressive8ChNrtVcapVencVdecVdisSetCapFrameRate(
        VCAP_CHN  vcChnId, 
        VCAP_STRM vStrmId, 
        ...
        Int32     outputFrameRate)
{
    ...
    switch (vStrmId)
    {
        case 0: /* preview path */
            ...
            //break;
        case 1: /* primary path */
            //printf("ATTENTION: Input frame rate is fixed to 15 fps(NTSC) or 12 fps(PAL)\n inputFrameRate will be forced to 15/12 fps!!\n");
            if (outputFrameRate > inputFrameRate / 4)
            {
                printf("ATTENTION: Output frame should not be more than 15 fps(NTSC) or 12 fps(PAL)!!\n");
                return ERROR_NONE;
            }
            deiId = DEI_VIP0_SC_DEI_SC_2;
            params.chId = vcChnId;
            params.inputFrameRate  = inputFrameRate / 4;
...
Int32 MultiCh_progressive8ChNrtVcapVencVdecVdisEnableDisableCapChn(
        VCAP_CHN  vcChnId, 
        VCAP_STRM vStrmId, 
        Bool      enableChn)
{
...
Int32 MultiCh_progressive8ChNrtVcapVencVdecVdisSetOutRes(
        VCAP_CHN                  vcChnId, 
...
Int32 MultiCh_progressive8ChNrtVcapVencVdecVdisGetOutRes(
        VCAP_CHN                  vcChnId, 
        VCAP_CHN_DYNAMIC_PARAM_S *psCapChnDynaParam)
{
    DeiLink_chDynamicSetOutRes params = {0};
