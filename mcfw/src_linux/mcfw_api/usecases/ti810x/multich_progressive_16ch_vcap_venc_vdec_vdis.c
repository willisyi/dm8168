...
/*------------------------------ D1/CIF <h.264> + QCIF <h.264> + D1 <MJPEG> -------------------------------------
              ...
              -----------------------------------------------------------------------------------------------------------
               ...
               |           -------------------------                 |                       | 30fps                  |
16D1 420 30fps |              DEI_0  <name-DEI2>            ------------------               | (Even)                 |
...
#define TILER_ENABLE_SECONDARY_CHANNEL_ENCODE   FALSE
...
//#define DISABLE_SECONDARY_CHANNEL_ENCODE
#define DISABLE_1D1_ENCODE  /* and make it CIF */

//#define DECODE_8CH_ONLY
//#define CAPTURE_8CH_ONLY

#define NUM_SCD_CHANNELS    (16)

#define SCD_START_CH_ID     (16)

#define      ADD_DEI_FOR_1_AND_4CH_60_FPS
#define      ADD_NSF_AFTER_CAPTURE
#define      NO_NOTIFY_MODE TRUE
...
#define     NUM_PLAYBACK_CH            16
#define     NUM_DEI_LIVE_CH            4
#define     NUM_DEI_D1_CH              1
#define     NUM_DEI_CIF_CH              15

#ifdef CAPTURE_8CH_ONLY

#undef NUM_LIVE_CH
#undef NUM_DEI_CIF_CH
#undef SCD_START_CH_ID
#undef NUM_SCD_CHANNELS
#undef NUM_DEI_D1_CH
#undef NUM_DEI_CIF_CH

#define SCD_START_CH_ID            8
            ...
            )
{
    ...
    /* disabling tiler for +1D1 encode since otherwise tiler memory is not enough for all other functions */
    deiPrm->tilerEnable[DEI_LINK_OUT_QUE_VIP_SC] = FALSE;

    #ifdef DISABLE_1D1_ENCODE
    /* bypass DEI - only even fields are scaled  */
    deiPrm->enableDeiForceBypass = TRUE;

    /* QCIF width == width of a field/4 */
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_DEI_SC][0].ratio.widthRatio.denominator = 4;

    /* QCIF height == height of a field/2   */
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_DEI_SC][0].ratio.heightRatio.denominator = 2;

    /* CIF width == width of a field/2 */
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][0].ratio.widthRatio.denominator = 2;

    /* CIF height == height of a field   */
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][0].ratio.heightRatio.denominator = 1;

    /* no skipping of fields - odd fields skipped automatically when enableDeiForceBypass = TRUE */
    deiPrm->inputDeiFrameRate  = 60;
            ...
            )
{
    ...
    /*
        Below params set in set_DEI_VIP0_SC_1D1_CH_params()

        set_DEI_VIP0_SC_1D1_CH_params() MUST be called before this API
    ...
    */
    ...
    for(chId=0; chId<selectPrm->outQueChInfo[1].numOutCh; chId++)
    {
        selectPrm->outQueChInfo[1].inChNum[chId] = chId+NUM_DEI_D1_CH;
    }
    ...
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][0].ratio.heightRatio.numerator = 1;
    deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][0].ratio.heightRatio.denominator = 1;
    for (chId=NUM_DEI_D1_CH; chId < NUM_DEI_CIF_CH; chId++)
        deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][chId] = deiPrm->outScaleFactor[DEI_LINK_OUT_QUE_VIP_SC][0];
...
static void selectCaptureFullFpsChs(UInt32 numChs, UInt32 fullFpsChs[])
{
    ...
    {
        prm.oddFieldSkipRatio = CAPTURE_LINK_ODD_FIELD_SKIP_1_4;
    }
...
static void setDeiEvenOddEvenPatternDei(UInt32 numChs, UInt32 fullFpsChs[])
{
    DeiLink_ChSetEvenOddEvenPatternDeiParams prm;
    UInt32 chId;
    UInt32 chBitMask = 0xFFFF;
    DeiLink_ChFpsParams deiOutFrameRate;
    
    deiOutFrameRate.streamId = DEI_LINK_OUT_QUE_VIP_SC;

    for (chId=0; chId<numChs; chId++)
    {
        chBitMask &= ~(1<<fullFpsChs[chId]);
    }
    
    for (chId=0; chId<NUM_LIVE_CH; chId++)
    {
        prm.chId = chId;
        deiOutFrameRate.chId = chId;
        if (chBitMask&0x1)
        {
            prm.enable = TRUE;
          
            deiOutFrameRate.inputFrameRate  = 7;
            deiOutFrameRate.outputFrameRate = 7;
        }
        else
        {
            prm.enable = FALSE;

            deiOutFrameRate.inputFrameRate  = 11;
            deiOutFrameRate.outputFrameRate =  7;
        }
        System_linkControl(
        ...
        System_linkControl(
            ...
            &deiOutFrameRate,
            sizeof(deiOutFrameRate),
            TRUE
            );
        chBitMask = chBitMask>>1;
    }
}
...
//    if (gVsysModuleContext.vsysConfig.systemUseCase == VSYS_USECASE_MULTICHN_PROGRESSIVE_VCAP_VDIS_VENC_VDEC_D1_AND_CIF)
    ...
    {
        setDeiEvenOddEvenPatternDei(numSelectChs, selectChs);
    }
...
#ifdef DISABLE_SECONDARY_CHANNEL_ENCODE
static void disableSecondaryStream()
{
    UInt32 deiId, chId;
    DeiLink_ChannelInfo prm;

    deiId = gVcapModuleContext.deiId[DEI_BYPASS_VIP1_SC_SC2];
    ...
    deiId = gVcapModuleContext.deiId[DEI_VIP0_SC_DEI_SC_FOR_1D1];
...
Void MultiCh_createProgressive16ChVcapVencVdecVdis()
{
    ...
    #ifdef CAPTURE_8CH_ONLY
    gVencModuleContext.vencConfig.numPrimaryChn = 8;
    gVencModuleContext.vencConfig.numSecondaryChn = 8;
    #endif
...
#if     ((TILER_ENABLE == FALSE) && (TILER_ENABLE_SECONDARY_CHANNEL_ENCODE == FALSE))
    ...
    nsfPrm[NSF_AFTER_CAPTURE].outQueParams[0].nextLink      = dupId[CAPTURE_DUP_LINK_IDX];
    nsfPrm[NSF_AFTER_CAPTURE].numBufsPerCh                     = NUM_NSF_BUFFERS;
#endif


#ifdef ADD_NSF_AFTER_CAPTURE
    dupPrm[CAPTURE_DUP_LINK_IDX].inQueParams.prevLinkId           = gVcapModuleContext.nsfId[NSF_AFTER_CAPTURE];
#else
    ...
    dupPrm[CAPTURE_DUP_LINK_IDX].outQueParams[2].nextLink       = gVcapModuleContext.sclrId[0];
    dupPrm[CAPTURE_DUP_LINK_IDX].numOutQue                      = 3;
#ifdef      ADD_DEI_FOR_1_AND_4CH_60_FPS
    dupPrm[CAPTURE_DUP_LINK_IDX].outQueParams[3].nextLink       = selectId[DEI_LIVE_SELECT];
    ...
    set_DEI_BYPASS_VIP1_SC_SC2_params(
                            &selectPrm[DEI_1_D1_CH_SELECT], &deiPrm[DEI_BYPASS_VIP1_SC_SC2],
                            dupId[CAPTURE_DUP_LINK_IDX], 1,
    ...
    nsfPrm[NSF_AFTER_BYPASS_DEI_SC_LINK].tilerEnable                          = TILER_ENABLE_SECONDARY_CHANNEL_ENCODE;
    ...
    nsfPrm[NSF_FOR_1QCIF].tilerEnable                          = TILER_ENABLE_SECONDARY_CHANNEL_ENCODE;
...

    {
        ...
        ipcBitsOutHostPrm.inQueInfo.numCh = NUM_PLAYBACK_CH;
    ...
    ipcBitsOutHostPrm.numBufPerCh[1]  = NUM_IPC_OUT_BUFFERS;
...
Int32 MultiCh_progressive16ChVcapVencVdecVdisSwitchLayout(VDIS_DEV vdDevId, SwMsLink_LayoutPrm *pSwMsLayout)
{
    ...
    */
    ...
    {
        /* Dont select CHs for SD display, since HDMI will select the CHs and SD display should not override them */
...
//            printf ("Preview path frame rate change not possible\n");
        ...
        case 2:
            ...
            if (vcChnId == 0)
            {
                deiIdx = DEI_VIP0_SC_DEI_SC_FOR_1D1;
                params.chId = vcChnId;
            }
            else
            {
                deiIdx = DEI_BYPASS_VIP1_SC_SC2;
                params.chId = vcChnId - 1;
            }
            if(gVcapModuleContext.deiId[deiIdx]!=SYSTEM_LINK_ID_INVALID)
            {
                status = System_linkControl(gVcapModuleContext.deiId[deiIdx], DEI_LINK_CMD_SET_FRAME_RATE,
    ...
    switch (vcStrmId)
    {
        ...
        case 2:
            ...
            if (vcChnId == 0)
                ...
                channelInfo.channelId = vcChnId - 1;
