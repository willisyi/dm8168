...
SECTIONS
{
  ...
  .bssNonCachedHDVPSS :
  {
    *(.bss:extMemNonCache:heap)

  } > HDVPSS_DESC_MEM, type = NOLOAD


  .bssNonCachedRemoteCoreShMem :
  {
    *(.bss:extMemNonCache:remoteDebugCoreShm)

  } > REMOTE_DEBUG_MEM

/*bssProxyServer and bss should either map to the same section or map to two different
  sections which are not translations of each other i.e physically distinct*/
 // .bssProxyServer:
//  {
//   -lhdvpss_proxyServer.aem3(.bss)
//  } > DDR3_M3 ,type = NOLOAD

  .bss:
  {
   *(.bss)
  } > DSS_M3_BSS_ADDR_RUN, type = NOLOAD
